...
The first runnable slice includes deterministic replay and fake transports, strict request allow-listing, generic PID/DTC/readiness decoders, atomic JSON session storage, Markdown/HTML evidence reports, conservative observations, and a Windows desktop replay viewer. A separately reviewed Classic Bluetooth COM-port transport is now available for supervised integration work; the viewer still starts with synthetic data and never connects automatically.
